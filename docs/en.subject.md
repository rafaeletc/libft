...
Return value | None |
External functs. | free |
Description | Deletes and frees the given element and every successor of that element, using the function 'del' and free(3). <br> Finally, the pointer to the list must be set to NULL. |
...
External functs. | `malloc`, `free` |
Description | Iterates the list 'lst' and applies the function 'f' to the content of each element. Creates a new list resulting of the successive applications of the function 'f'. The 'del' function is used to delete the content of an element if needed. <br> You are free to add any function to your libft as you see fit. |
